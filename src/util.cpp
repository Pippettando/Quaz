...
const char * const BITCOIN_CONF_FILENAME = "quaz.conf";
const char * const BITCOIN_PID_FILENAME = "quazd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "quaz" is a composite category enabling all Quaz-related debug output
            if(ptrCategory->count(string("quaz"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuazCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuazCore
    // Mac: ~/Library/Application Support/QuazCore
    // Unix: ~/.quazcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuazCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuazCore";
#else
    // Unix
    return pathRet / ".quazcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty quaz.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override quaz.conf
